...
export const meadowcap: Expression = site_template(
  ...
  [
    ...
    hsection("meadowcap_overview", "Overview", [
      ...
      figure(
        img(asset("meadowcap/capability_attempts.png"), `A two-column comic. The left column first shows Alfie handing a neat slip of paper to Betty. The second panel shows Betty inspecting the paper with a magnifying glass. The magnified text clearly reads "signed: Gemma". In the final panel, a happy Betty hands a box over to a happy Alfie. The right column depicts a less fruitful interaction. A cartoonish troll approaches Betty with a crumpled paper sheet. When Gemma inspects it in the second panel, it reads "i AM StiNKY GEMA", clearly not Gemma’s real signature. In the final panel, Betty tells the troll off, not handing anything over.`),
      ),
      ...
      marginale([
        img(asset("meadowcap/communal_namespace.png"), `A front view of a stylized house. The house has three separate entries, each in a different colour. Above each entry is a window in a matching color, each with some happy denizens sticking their heads out. The outer windows contain a single person, the middle window is shared by two people.`),
      ...
      marginale([
        img(asset("meadowcap/owned_namespace.png"), "A similar front view of a house, with windows showing the inhabitants. Unlike the preceding drawing, this house only a single door, color coded in orange. The three windows each show a combination of orange and an individual color per window. In front of the window stands the owner, dutifully (and cheerfully) maintaining the lawn with a broom."),
    ...
    hsection("capabilities", "Capabilities", [
      ...
      pinformative(
        marginale(img(asset("meadowcap/capability_semantics.png"), `A neat piece of paper, styled like an admission ticked, with a heading saying "This Capability Grants...". The heading is followed by four sections. The first section states the receiver as "Alfie", the second section states the granting of "read access", the third section gives a time range as "to all messages from last week", and finally a large stamp mark simply saying "valid".`)),
    ...
    img(asset("meadowcap/meadowcap.png"), `A Meadowcap emblem: A stylized drawing of two meadowcaps (a type of fungus), next to a hand-lettered cursive of the word "Meadowcap".`),
  ],
);
