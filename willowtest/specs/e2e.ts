...
aside_block,
  ...
  export const e2e: Expression = site_template({
    ...
    hsection("e2e_paths", "Paths", [
      ...
      pinformative("We can achieve both hierarchic decryption capabilities and prefix-preservation by encrypting individual ", rs("Component"), " with encryption keys that we successively derive with a ", link("key derivation function", "https://en.wikipedia.org/wiki/Key_derivation_function"), ". Let ", def_value("kdf"), " be a key-derivation function that takes as inputs an encryption key and a byte string of length at most ", r("max_component_length"), ", and that returns a new encryption key.", marginale(["A suitable candidate for ", r("kdf"), " is ", link("HKDF (RFC 5869)", "https://datatracker.ietf.org/doc/html/rfc5869"), ", using successive ", rs("Component"), " as the salt of each derivation step."]), " Given an initial key ", def_value({id: "kdf_k0", singular: "key_0"}), ", we can then encrypt ", rs("Path"), " as follows:", lis(
        ["Encrypting the empty ", r("Path"), " yields the empty ", r("Path"), " again."],
        ["Encrypt ", rs("Path"), " with a single ", r("Component"), " ", def_value({id: "kdf_cmp0", singular: "component_0"}), " using the key ", code(function_call(r("kdf"), r("kdf_k0"), r("kdf_cmp0"))), "."],
        ["For any other (longer) ", r("Path"), " , denote its final ", r("Component"), " as ", def_value({id: "kdf_cmpi", singular: "component_i"}), ", the ", r("Path"), " formed by its prior ", rs("Component"), " as ", def_value({id: "kdf_pi", singular: "path_i"}), ", and denote the key that is used to encrypt the final ", r("Component"), " of ", r("kdf_pi"), " as ", def_value({id: "kdf_ski", singular: "key_i"}), ". You then encrypt the ", r("Path"), " by first encrypting ", r("kdf_pi"), ", and then appending ", code(function_call(r("kdf"), r("kdf_ski"), r("kdf_cmpi"))), " as the final ", r("Component"), "."],
      )),
      ...
      pinformative("Giving the ability to decrypt all ", rs("Path"), " starting with some ", r("Path"), " ", def_value({id: "kdf_p", singular: "p"}), " requires three pieces of information: the plaintext of ", r("kdf_p"), ", the cyphertext of ", r("kdf_p"), ", and the secret key that was used to encrypt the final ", r("Component"), " of ", r("kdf_p"), "."),

      pinformative("When ", link_name("e2e_payloads", "encrypting payloads"), ", you can use the same key that encrypts the final ", r("Component"), " of an ", r("Entry"), "’s ", r("entry_path"), " for encrypting the ", r("Entry"), "’s ", r("Payload"), " as well (using ", r("kdf_k0"), " for encrypting ", rs("Payload"), " at the empty ", r("Path"), "). This scheme then conveys the same hierarchical decryption capabilities as those for ", rs("Path"), "."),
    ...
    hsection("e2e_auth", "AuthorisationTokens", [
