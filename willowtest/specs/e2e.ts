...
aside_block,
  ...
  export const e2e: Expression = site_template({
    ...
    hsection("e2e_spaceids", "NamespaceIds and SubspaceIds", [
      pinformative("If ", rs("NamespaceId"), " or ", rs("SubspaceId"), " were encrypted differently per ", r("subspace"), ", then ", rs("Entry"), " that should overwrite each other might not do so. Encrypting them on a per-namespace basis has no effect on the inferences that an observer can make about which entries are part of the same ", rs("NamespaceId"), " or ", rs("SubspaceId"), ". Hence, there is little reason to encrypt them."),

      pinformative("Any non-random-looking information that is part of ", rs("NamespaceId"), " or ", rs("SubspaceId"), " should then be considered as public. If user-friendly Ids are desired, one can still present meaningful Ids on the UI layer, but hash them in order to obtain the actual Ids that Willow uses internally. This is computationally less expensive than encrypting them."),
    ]),

    hsection("e2e_paths", "Paths", [
      pinformative(rs("Path"), " are arguably the most interesting facet of encrypting Willows. Since users (or their applications) assign paths to allow for meaningful access, ", rs("Path"), " carry information that clearly should be encrypted."),

      pinformative("Because ", rs("Entry"), " in the same ", r("subspace"), " overwrite each other based on their ", rs("entry_path"), ", the cyphertexts of two ", rs("Path"), " that begin with some number of equal components must begin with as many equal components as well. Such ", em("prefix-preserving encryption"), " ", sidenote("schemes", [link(`Fan, Jinliang, et al. "Prefix-preserving IP address anonymization: measurement-based security evaluation and a new cryptography-based scheme." Computer Networks 46.2 (2004): 253-272.`, "https://citeseerx.ist.psu.edu/document?repid=rep1&type=pdf&doi=3d6387c053cc9f3fe153bbffc79b492f0775c354")]), " are well-understood, and leak less information than order-preserving encryption schemes."),

      pinformative("The hierarchical nature of ", rs("Path"), " suggests another requirement: it should be possible to convey the ability to decrypt all ", rs("Entry"), " whose ", r("entry_path"), " starts with a particular prefix. For example, allowing somebody to decrypt ", path("blog", "ideas"), " should allow them to decrypt ", path("blog", "ideas", "game.txt"), " and ", path("blog", "ideas", "pets", "maze.txt"), ", but neither ", path("blog", "config.txt"), " nor ", path("code", "willow.rs"), "."),

      pinformative("We can achieve both hierarchic decryption capabilities and prefix-preservation by making use of a ", link("key derivation function", "https://en.wikipedia.org/wiki/Key_derivation_function"), ". Let ", def_value("kdf"), " be a key-derivation function that takes as inputs some secret key and a byte string of length at most ", r("max_component_length"), ", and that returns another secret key.", marginale(["A suitable candidate for ", r("kdf"), " is ", link("HKDF (RFC 5869)", "https://datatracker.ietf.org/doc/html/rfc5869"), ", using ", r("Path"), " components as the salt of each derivation step."]), " Then we can encrypt ", rs("Path"), " as follows:", lis(
        ["Encrypt the empty ", r("Path"), " using some initial secret key ", def_value({id: "kdf_k0", singular: "sk_0"}), "."],
        ["Encrypt ", rs("Path"), " with a single component ", def_value({id: "kdf_cmp0", singular: "component_0"}), " using the secret key ", function_call(r("kdf"), r("kdf_k0"), r("kdf_cmp0")), "."],
        ["For any other (longer) ", r("Path"), " , denote its final component by ", def_value({id: "kdf_cmpi", singular: "component_i"}), ", the ", r("Path"), " formed by its prior components as ", def_value({id: "kdf_pi", singular: "path_i"}), ", and denote the secret key that is used to encrypt the final component of ", r("kdf_pi"), " as ", def_value({id: "kdf_ski", singular: "sk_i"}), ". You then encrypt the ", r("Path"), " by first encrypting ", r("kdf_pi"), ", and then appending ", function_call(r("kdf"), r("kdf_ki"), r("kdf_cmpi")), " as the final component."],
      )),
      ...
      pinformative("When ", link_name("e2e_payloads", "encrypting payloads"), ", you can use the same key that encrypts the final component of an ", r("Entry"), "’s ", r("entry_path"), " for encrypting the ", r("Entry"), "’s ", r("payload"), " as well. This scheme then conveys the same hierarchical decryption capabilities as those for ", rs("Path"), "."),
