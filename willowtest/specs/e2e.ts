...
aside_block,
  ...
  export const e2e: Expression = site_template({
    ...
    hsection("e2e_paths", "Path", [
      ...
      pinformative("Because ", rs("Entry"), " in the same ", r("subspace"), " overwrite each other based on their ", rs("entry_path"), ", the cyphertexts of two ", rs("Path"), " that begin with some number of equal ", rs("Component"), " must begin with as many equal ", rs("Component"), " as well. Such ", em("prefix-preserving encryption"), " ", sidenote("schemes", [link(`Fan, Jinliang, et al. "Prefix-preserving IP address anonymisation: measurement-based security evaluation and a new cryptography-based scheme." Computer Networks 46.2 (2004): 253-272.`, "https://citeseerx.ist.psu.edu/document?repid=rep1&type=pdf&doi=3d6387c053cc9f3fe153bbffc79b492f0775c354")]), " are well-understood, and leak less information than order-preserving encryption ", sidenote("schemes", [link(`Li, Jun, and Edward R. Omiecinski. "Efficiency and security trade-off in supporting range queries on encrypted databases." IFIP Annual Conference on Data and Applications Security and Privacy. Berlin, Heidelberg: Springer Berlin Heidelberg, 2005.`, "https://repository.gatech.edu/server/api/core/bitstreams/ca0b7d1f-b382-446b-be32-1016a4e443e4/content")]), "."),
      ...
      pinformative("We can achieve both hierarchic decryption capabilities and prefix-preservation by encrypting individual ", rs("Component"), " with encryption keys that we successively derive with a ", link("key derivation function", "https://en.wikipedia.org/wiki/Key_derivation_function"), ". Let ", def_fn("kdf"), " be a key-derivation function that takes as inputs an encryption key and a byte string of length at most ", r("max_component_length"), ", and that returns a new encryption key.", marginale(["A suitable candidate for ", r("kdf"), " is ", link("HKDF (RFC 5869)", "https://datatracker.ietf.org/doc/html/rfc5869"), ", using successive ", rs("Component"), " as the salt of each derivation step."]), " Let ", def_fn({id: "path_encrypt_component", singular: "encrypt_component"}), " be a function that encrypts a single ", r("Component"), " with a given encryption key.", " Given an initial key ", def_value({id: "kdf_k0", singular: "key_0"}), ", we can then encrypt ", rs("Path"), " as follows:", lis(
        ["Encrypting the empty ", r("Path"), " yields the empty ", r("Path"), " again."],
        ["Encrypt a ", r("Path"), " with a single ", r("Component"), " ", def_value({id: "kdf_cmp0", singular: "component_0"}), " as ", function_call(r("path_encrypt_component"), r("kdf_k0"), r("kdf_cmp0")), "."],
        ["For any other (longer) ", r("Path"), " , denote its final ", r("Component"), " as ", def_value({id: "kdf_cmpipp", singular: "component_ipp"}), ", the ", r("Path"), " formed by its prior ", rs("Component"), " as ", def_value({id: "kdf_pi", singular: "path_i"}), ", the final component of ", r("kdf_pi"), " as ", def_value({id: "kdf_cmpi", singular: "component_i"}), ", and denote the key that is used to encrypt ", r("kdf_cmpi"), " as ", def_value({id: "kdf_ski", singular: "key_i"}), ". You then encrypt the ", r("Path"), " by first encrypting ", r("kdf_pi"), ", and then appending ", function_call(
      ...
      pinformative("This technique produces prefix-preserving cyphertexts, and it lets anyone who can decrypt a certain ", r("Path"), " also decrypt all extensions of that ", r("Path"), ". As long as the ", r("kdf"), " is non-invertable even for known ", rs("Component"), ", no other ", rs("Path"), " can be decrypted beyond the point where they differ."),

      pinformative("Giving the ability to decrypt all ", rs("Path"), " starting with some ", r("Path"), " ", def_value({id: "kdf_p", singular: "p"}), " requires three pieces of information: the plaintext of ", r("kdf_p"), ", the cyphertext of ", r("kdf_p"), ", and the encryption key that is used for encrypting ", rs("Path"), " that have exactly one more ", r("Component"), " than ", r("kdf_p"), "."),

      pinformative("When ", link_name("e2e_payloads", "encrypting payloads"), ", you can use the same key that encrypts the final ", r("Component"), " of an ", r("Entry"), "’s ", r("entry_path"), " for encrypting the ", r("Entry"), "’s ", r("Payload"), " as well (leaving ", rs("Payload"), " at the empty ", r("Path"), " unencrypted). This scheme then conveys the same hierarchical decryption capabilities as those for ", rs("Path"), "."),
    ...
    hsection("e2e_auth", "AuthorisationToken", [
      pinformative("Peers store and exchange not only ", rs("Entry"), " but ", rs("AuthorisedEntry"), ". Whether ", rs("AuthorisationToken"), " can be meaningfully encrypted depends on the choice of ", r("is_authorised_write"), ", and should be taken into account when designing and using these parameters. ", link_name("meadowcap", "Meadowcap"), " ", rs("Capability"), " cannot be encrypted, as this would remove the ability of peers without access to the decryption keys to verify the ", rs("dss_signature"), "."),
    ]),
  ]);
