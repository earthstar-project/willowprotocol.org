...
export const access_control: Expression = site_template(
  ...
  [
    pinformative("In this document, we describe how peers can control who gets to request which ", rs("Entry"), ". The basic idea is simple: you only answer requests for ", rs("Entry"), " if the request comes with some proof that the original author has allowed the requesting peer to access the data."),
    ...
    pinformative("For a capability system to be usable for ", r("read_authentication"), " it needs to provide three types of semantics", marginale([link_name("meadowcap", "Meadowcap"), ", our bespoke capability system for Willow, just so happens to provide these semantics."]), ": each ", r("read_capability"), " must have a single ", def({ id: "access_receiver", singular: "receiver" }), " (a ", r("dss_pk"), " of some ", r("signature_scheme"), "), it must have a single ", def({ id: "granted_area", singular: "granted area" }), " (an ", r("Area"), "), and it must have a single ", def({ id: "granted_namespace", singular: "granted namespace" }), " (a ", r("NamespaceId"), "). Access control can then be implemented by answering only requests for ", rs("Entry"), " in the ", r("granted_namespace"), " and ", r("granted_area"), " of some ", r("read_capability"), " whose ", r("access_receiver"), " is the peer in question."),

    pinformative("Peers can prove that they are the ", r("access_receiver"), " of a ", r("read_capability"), " by supplying a valid ", r("dss_signature"), " over some data that has never been signed by that ", r("dss_pk"), " before (this prevents replaying of ", rs("dss_signature"), "). To obtain such a piece of data (a ", link("nonce", "https://en.wikipedia.org/wiki/Cryptographic_nonce"), "), the peers can simply generate a sufficiently large random number. Thankfully, collaboratively generating a random number — even if one of the peers might be malicious — is a solved problem."),

    marginale(img(asset("access_control/recommended_proof_technique.png"), `A comic visualizing nonce generation with one column for each peer. The peers think of their numbers, hash them, exchange the hashes, then exchange their numbers, verify the hash of the received number, xor the numbers together, with one peer using the complement number instead. With the nonces generated, the peers then each sign their nonce, echange the signatures, and verify the signature they received. After successful verification, they start chattering in binary.`)),
