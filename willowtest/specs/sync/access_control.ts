...
export const access_control: Expression = site_template(
  ...
  [
    ...
    pinformative("As closing thoughts, we want to compare this way of controlling read access with that of encrypting data and passing it around freely. Both approaches keep data inaccessible to unauthorized agents as long as the underlying cryptographic primitives remain strong. Freely sharing encrypted data has the drawback that all data becomes public once the encryption primitive is broken, whereas peer could refuse to pass on any data once the signature scheme is broken. Then again, encrypted data can readily propagate through an untrusted peer-to-peer network, whereas access-controlled unencrypted data can only travel between peers with access."),
