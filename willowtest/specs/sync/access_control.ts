...
export const access_control: Expression = site_template(
    ...
    [
        pinformative("The ", r("wgps"), " includes mechanisms for access control: peers only hand out data to recipients who can prove that they were granted access to that data. This document describes the concepts this involves."),

        pinformative(def({ id: "read_authentication", singular: "read authentication"}, "Read authentication"), " is the process of one peer proving that they have access to some data. We employ ", def({ id: "read_capability", singular: "read capability", plural: "read capabilities"}, "read capabilities"), ", unforgeable tokens that grant access to some data to the holder of some secret key. The ", r("wgps"), " does not specify a particular capability scheme. We recommend using our bespoke ", link_name("meadowcap", "meadowcap"), " system, but this is not a hard requirement."),

        pinformative("For a capability system to be usable for ", r("read_authentication"), " it needs to provide two types of semantics: each ", r("read_capability"), " must have a single ", def({ id: "access_receiver", singular: "receiver"}), " — a public key of some signature scheme — and it must have a single ", def({ id: "granted_product", singular: "granted product"}), " — a ", r("3d_product"), ". The ", r("wgps"), " makes peers peers transmit ", rs("read_capability"), " of which they are the ", r("access_receiver"), ", and restricts them to requesting only ", rs("entry"), " that lie in the ", r("granted_product"), " of one of the capabilities they registered."),

        pinformative("In order to prove to each other that they are need the intended receivers of the", rs("read_capabilities"), " the peers registered, they need to produce a valid signature for the ", r("access_receiver"), " of each capability. To prevent them from replaying signatures they might have picked up earlier, the data they sign has to be a ", link("nonce", "https://en.wikipedia.org/wiki/Cryptographic_nonce"), ". Collectively finding a nonce even if one of the peers might be malicious is fortunately a solved problem."),
        ...
        pinformative("Which choice is more appropriate for which use case cannot be answered in general, and the best choice might be a hybrid option. We merely supply a tool for restricting how data propagates, but it is up to you to decide how to use that tool. The most important thing is that you carefully consider options and implications before making that decision."),
    ],
);