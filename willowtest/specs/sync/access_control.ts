...
export const access_control: Expression = site_template(
    ...
    [
        ...
        pinformative("For a capability system to be usable for ", r("read_authentication"), " it needs to provide three types of semantics: each ", r("read_capability"), " must have a single ", def({ id: "access_receiver", singular: "receiver"}), " (a public key of some signature scheme), it must have a single ", def({ id: "granted_area", singular: "granted area"}), " (an ", r("area"), "), and it must have a single ", def({ id: "granted_namespace", singular: "granted namespace"}), " (a ", r("namespace_id"), "). The ", r("wgps"), " makes peers transmit ", rs("read_capability"), " of which they are the ", r("access_receiver"), ", and restricts peers to requesting only ", rs("entry"), " that are ", r("area_include", "included"), " in the ", r("granted_area"), " and ", r("granted_namespace"), " of one of the capabilities they registered."),
