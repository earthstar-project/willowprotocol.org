    ...
    hsection("resources_data_handles", "Data Handles", [
      ...
      hsection("resources_handles_managing", "Managing Resource Handles", [
        pinformative(
          ...
          ". For each ",
          r("handle_type"),
          ", both peers keep track of the least number that has not yet been assigned as a ",
          r("resource_handle", "handle"),
          " of that ",
          r("handle_type", "type"),
          ". In order to ",
          r("handle_bind"),
          ...
          ", the ",
          r("handle_client"),
          " simply transmits the data and its type. The least number that had not been assigned before becomes the ",
          r("resource_handle"),
          " of the transmitted data. Both peers add the new mapping into some data structure they can use to later dereference the ",
          r("resource_handle"),
          " into its value.",
        ),

        pinformative(
          "The main issue in maintaining ",
          rs("resource_handle"),
          " is that of coordinating ",
          r("handle_free", "freeing"),
          " over an asynchronous communication channel. Suppose one peer removes a handle-to-value binding from their local mapping, sends a message to inform the other peer, but that peer has concurrently sent some message that pertains to the ",
          r("resource_handle"),
          " in question. Everything breaks! Hence, ",
          r("handle_free", "freeing"),
          " must be a three-step process:",
        ),

        ols(
          [
            "one peer sends a message that proposes to ",
            r("handle_free"),
            " a ",
            r("resource_handle"),
            " (and, in doing so, that peer commits to not referring to that ",
            r("resource_handle", "handle"),
            " in any of its future messages),",
          ],
        ...
        pinformative(
          "There is no need to use separate message types for proposals and confirmations of ",
          r("handle_free", "freeing"),
          ": peers react to a proposal to ",
          r("handle_free"),
          " by sending their own proposal to ",
          r("handle_free"),
          " the same ",
          r("resource_handle"),
          ". Receiving a proposal to ",
          r("handle_free"),
          " a ",
          r("resource_handle"),
          " after having sent such a proposal oneself confirms that the handle-to-value binding can be removed from the local mapping. A nice side effect of this approach is that it gracefully handles concurrent proposals to ",
          r("handle_free"),
          " the same ",
          r("resource_handle"),
          " by both peers.",
