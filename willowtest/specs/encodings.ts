...
export const encodings: Expression = site_template({
  ...
  pinformative(
    "A perhaps curious feature of the Willow data model is that its specifications never talks about encodings. ",
    sidenote("We", ["Let", apo, " be honest: ", em("Aljoscha")]),
    " strongly believe that specifications should concern themselves with purely logical data types as long as possible, treating encodings as a minor and ultimately interchangeable detail. When specifications define concepts in terms of their encodings, results usually end up miserably underspecified (see ",
    link("JSON", "https://en.wikipedia.org/wiki/JSON#Interoperability"),
    ") or full of incidental complexity (see ",
    link("XML", "https://en.wikipedia.org/wiki/XML"),
    ").",
  ),
  ...
  hsection("encodings_what", "Encodings, In Detail", [
    pinformative(
      "The Willow protocols are generic over user-supplied parameters. Invariably, some values of the user-supply types need to be encoded, so there also have to be user-supplied definitions of how to encode things. But not every function that turns values into byte strings defines an encoding.",
    ),
  ...
  hsection("specific_encodings", "Specific Encodings", [
    ...
    pinformative(
      "When designing encodings, there rarely exists a single ",
      em("best"),
      " solution. Different scenarios might warrant different trade-offs between encoding and decoding time, encoding size, or simplicity. Furthermore, additional context can often enable more efficient encodings than general-purpose solutions. For example, a database that groups ",
      rs("Entry"),
      " by ",
      r("entry_namespace_id"),
      " would not need to encode the ",
      r("entry_namespace_id"),
      " of every individual ",
      r("Entry"),
      ".",
    ...
    hsection("encodings_data_model", "Data Model Encodings", [
      pinformative(
        "When encoding ",
        rs("Path"),
        ", we want to use fixed-width unsigned integers of minimal width. Hence, we define:",
      ),

      pinformative(
        def_value("path_length_power"),
        " is the least natural number such that ",
        code("256^", r("path_length_power"), " >= ", r("max_component_length")),
        ". We can represent the length of any ", r("Component"), " in ",
        r("path_length_power"),
        " bytes. ",
        def_type("UPathLengthPower"),
        " denotes the type of numbers between zero (inclusive) and ",
        code("256^", r("path_length_power")),
        " (exclusive).",
      ),

      pinformative(
        def_value("path_count_power"),
        " is the least natural number such that ",
        code("256^", r("path_count_power"), " >= ", r("max_component_count")),
        ". We can represent the number of ", rs("Component"), " of any ",
        r("Path"),
        " in ",
        r("path_count_power"),
        " bytes. ",
        def_type("UPathCountPower"),
        " denotes the type of numbers between zero (inclusive) and ",
        code("256^", r("path_count_power")),
        " (exclusive).",
      ),
      ...
      pinformative(
        ...
        r("Path"),
        " ",
        def_value({ id: "enc_path_p", singular: "p" }),
        ", we define ",
        function_call(def_value("encode_path"), r("enc_path_p")),
        " as the concatenation of",
        lis(
          [
            "the number of ", rs("Component"), " of ",
            r("enc_path_p"),
          ...
          [
            "for each ", r("Component"), " of ",
            r("enc_path_p"),
            ",",
            lis(
              [
                "the length of the ", r("Component"), ", encoded as an unsigned, big-endian ",
                r("path_length_power"),
                "-byte integer, followed by the bytes of the ", r("Component"), ".",
              ],
            ),
          ],
        ),
      ...
      pinformative(
        ...
        r("Entry"),
        " ",
        def_value({ id: "enc_entry_e", singular: "e" }),
        ", let ",
        def_value("encode_namespace_id"),
        " be an ",
        r("encoding_function"),
        " for ",
        r("NamespaceId"),
        ", let ",
        def_value("encode_subspace_id"),
        " be an ",
        r("encoding_function"),
        " for ",
        r("SubspaceId"),
        ", and let ",
        def_value("encode_payload_digest"),
        " be an ",
        r("encoding_function"),
        " for ",
        r("PayloadDigest"),
        ". We then define ",
        function_call(def_value("encode_entry"), r("enc_entry_e")),
        " as the concatenation of",
        lis(
          [
            function_call(
          ...
          [
            ...
            ", encoded as an unsigned, big-endian 64-bit integer,",
          ],
          [
      ...
      pinformative(
        "To encode small numbers with fewer bytes than large numbers, we define the for any number ",
        def_value({ id: "compact_width_n", singular: "n" }),
        " between zero (inclusive) and ",
        code("2^64"),
        " (exclusive) the value ",
        function_call(def_value("compact_width"), r("compact_width_n")),
        " as",
        lis(
    ...
    hsection("relativity", "Relativity", [
      pinformative(
        "When encoding several Willow objects, we can often achieve smaller outputs by encoding only how some object differs from another. In this section, we define several such relative encodings.",
      ),
      ...
      pinformative(
        ...
        ", we define ",
        function_call(
          def_value("encode_path_relative"),
        ...
        lis(
          ...
          [
            ...
            " of the ",
            r("Path"),
            " obtained by removing that longest common prefix from ",
            r("relative_path_primary"),
            ".",
          ],
        ),
      ...
      pinformative(
        "In all subsequent definitions, whenever the value ",
        r("range_open"),
        " is part of a numeric computation or comparison, it should be treated as a very large number, say, ",
        code("2^9999"),
