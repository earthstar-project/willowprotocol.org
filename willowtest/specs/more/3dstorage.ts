...
title: "Three-Dimensional Data Storage",
    ...
    pinformative("Most implementations of Willow will centre around a (persistent) data base of ", rs("Entry") , ". In this document, we give a bit of context on buiding such a data store. We cannot possibly give a complete account, think of this document more as a tour guide to an extensive, partially unexplored rabbit hole."),

    hsection("3dstorage_requirements", "Requirements", [
        ...
        pinformative("The bare minimum functionality that a Willow database has to provide to applications is the creation of new ", rs("Entry"), ", and retrieval of ", rs("Payload"), " by ", r("Entry"), ". The ", link_name("grouping_entries", "three-dimnsionality"), " of Willow suggests a natural way for applications to access data in bulk: by querying for all ", rs("Entry"), " in an ", sidenote(r("Area"), [
            "Queries for ", rs("3dRange"), " are not particularly meaningful in the face of end-to-end encrypted data; we recommend to always use ", rs("Area"), " in human-facing components such as programming APIs.",
        ]), "."),

        pinformative("As for the demands of syncing, we shall use the ", link_name("sync", "WGPS"), " as a (somewhat demanding) baseline. The WGPS requires the ability to compute ", rs("Fingerprint"), " for arbitrary ", rs("3dRange"), ", the ability to split arbitrary ", rs("3dRange"), " into multiple, roughly equally-sized subranges, and the ability to constrain ", rs("Area"), " to a number of newest ", rs("Entry"), " (to work with ", rs("AreaOfInterest"), ")."),

        pinformative("This gives us a fairly compact feature set, revolving around spatially constrained access to a three-dimensional arrangement of ", rs("Entry"), ". Unsurprisingly, there is quite a lot of prior work on spatial data structures."),
    ]),

    hsection("3dstorage_multidimensions", "Multidimensional Data Structures", [
        pinformative("Multidimensional search data structures have been studied for decades, whether in database research, computer graphics, geoinformatics, or several other fields of computer science. The bad news is that — unlike one-dimensional search data structures such as self-balancing search trees — no one-size-fits-all data structures with acceptable worst-case complexity profiles are known. The good news, however, is that this has not stopped decades worth of engineering power to develop systems that easily satisfy all practical requierments."),

        pinformative("Unfortunately, Willow has some peculiarities that make it hard to find and use suitable off-the-shelf solutions. While there exist several solutions for managing and querying multidimensional data indexed by fixed-width integers or floating point numbers, lexicographically sorted strings of strings (i.e., ", rs("Path"), ") are a different beast. ", r("Fingerprint"), " aggregation is another feature you will not find in SQLite."),
        ...
        pinformative("All of these data structures are tree-based, so they can readily be adapted to store the ", rs("Fingerprint"), " of the items of each subtree in each tree node, hence allowing for efficient ", r("Fingerprint"), " computation. Similarly, storing the total number of items in each node allows for efficiently working with ", rs("AreaOfInterest"), ", and can guide the splitting of ", rs("3dRange"), " for set reconciliation. Adopting the corresponding one-dimensional algorithms to the multidimensional tree structures is far from trivial, but ultimately doable. At the end of the day, a Willow implementation will thus rise and fall with the quality of its three-dimensional range queries."),

        pinformative("Engineering a three-dimensional data store is no easy feat, but the existence of numerous production-quality systems for working with multidimensional data shows that it is feasible. The whole design of Willow rests on this assumption, as well as on the assumption that three-dimensional data organisation suffices for building expressive and useful applications. While knowing full well that most contemporary peer-to-peer projects settle for more simple and less expressive data models, we believe that Willow hits a sweet(er) spot between expressivity and implementability."),
    ]),
