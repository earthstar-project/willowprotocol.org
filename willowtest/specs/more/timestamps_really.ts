...
When a Meadowcap capability restricts entry creation based on timestamps, this has ", em("absolutely nothing"), " to do with the (physical) time at which entries are ", em("created"), ". It merely restricts which times can be ", em("claimed"), " as the timestamp of entries.
...
The correct trade-off depends on the specific situation, it might even be impossible to know in advance. If this sounds discouraging, remember that centralised systems cannot tolerate ", em("any"), " period of disconnectivity.
...
The system of expiring capabilities does provide a good reason for future-dating entries: a moderator can irrevocably (for the original author) delete an entry by overwriting it with another entry whose timestamp is greater than the greatest time at which the original author can date their entries. Hence, peers should adjust the safeguard time difference they allow between new entries and their local clock to allow for moderations (for example by only rejecting entries whose time difference exceeds the longest issued capability lifetime by one extra minute). 
