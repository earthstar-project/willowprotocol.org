...
type RustDocsProps = {
  ...
  /** Where cached versions of rustdoc defs are stored. */
  cachingPath: string[];
};
...
export function DefsRustDocs(props: RustDocsProps): Expression {
  ...
  return (
    <impure
      fun={async (ctx) => {
        // Check if we already have these rustdocs.
        const defsCacheDirPath = join(Deno.cwd(), ...props.cachingPath);
        await ensureDir(defsCacheDirPath);

        const defsFileName = `${props.crate}-${
          props.json["crate_version"]
        }-defs.json`;

        const defsPath = join(defsCacheDirPath, defsFileName);

        const defsDict = await Deno.readTextFile(defsPath).then((
          jsonString,
        ) => (JSON.parse(jsonString) as Record<
          string,
          { url: string; name: string; kind: SymbolKind }
        >)).catch(async () => {
          logger.warn(
            ctx,
            `No cached defs found for ${props.crate} ${
              props.json["crate_version"]
            }, generating and saving...`,
          );

          const defs = createDefs(props.crate, props.json, props.prefix);

          await Deno.writeTextFile(defsPath, JSON.stringify(defs));
