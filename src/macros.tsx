import { A, Def,  Wip, Expression, Expressions, M } from "../deps.ts";
import { Span } from "../deps.ts";

////////////////////////
// Comments and TODOs //
////////////////////////

export function Alj({children, inline}: {children?: Expressions, inline?: boolean}): Expression {
...
export function AE({ children, href }: { children: Expressions, href: Expressions }): Expression {
  return <A clazz="external" href={<exps x={href}/>}><exps x={children}/></A>;
}

export function Orange({ children }: { children: Expressions }): Expression {
  return (
    <Span clazz="orange"><exps x={children}/></Span>
  );
}

export function SkyBlue({ children }: { children: Expressions }): Expression {
  return (
    <Span clazz="sky-blue"><exps x={children}/></Span>
  );
}

export function Green({ children }: { children: Expressions }): Expression {
  return (
    <Span clazz="green"><exps x={children}/></Span>
  );
}

export function Yellow({ children }: { children: Expressions }): Expression {
  return (
    <Span clazz="yellow"><exps x={children}/></Span>
  );
}

export function Blue({ children }: { children: Expressions }): Expression {
  return (
    <Span clazz="blue"><exps x={children}/></Span>
  );
}

export function Purple({ children }: { children: Expressions }): Expression {
  return (
    <Span clazz="purple"><exps x={children}/></Span>
  );
}

export function Vermillion({ children }: { children: Expressions }): Expression {
  return (
    <Span clazz="vermillion"><exps x={children}/></Span>
  );
}

export function Path({ components }: { components: Expression[] }): Expression {
  return <Span clazz="path">
    <exps x={components.map((comp, i) => <Span clazz="path_segment">
      <Span clazz="path_segment_txt">{comp}</Span>
    </Span>)}/>
  </Span>;
}

export function An({ an, capitalise }: { an?: boolean, capitalise?: boolean }): Expression {
  if (capitalise) {
    return an ? "An" : "A";
  } else {
    return an ? "an" : "a";
