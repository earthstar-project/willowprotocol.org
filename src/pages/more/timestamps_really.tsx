    ...
    <File name="index.html">
      ...
      >
        ...
        hsection("why_timestamps_anyways", "Why We Use Timestamps Anyways", [
            ...
            pinformative("Mapping a set of concurrently created <Rs n="Entry"/> to a single winner cannot be done in a fully satisfactory way, but we can at least try to improve the situation: by annotating every <R n="Entry"/> with its ", r("entry_timestamp"), ". ", em("If"), " the issuers are truthful and have sufficiently synchronised clocks, we get the expected outcome of the newest <R n="Entry"/> winning. That’s a lot better than taking a purist view and choosing, for example, the <R n="Entry"/> with the lowest ", r("entry_payload_digest"), ". In case of tied ", rs("entry_timestamp"), ", we can safely fall back to the hash option (and distinct ", rs("Payload"), " never hash to the same digest by assumption)."),
        ...
        hsection("rouge_timestamps", "Handling Rogue Timetamps", [
            ...
            pinformative("Willow automatically separates <Rs n="Entry"/> by ", r("entry_subspace_id"), ". An <R n="Entry"/> with a high ", r("entry_timestamp"), " only overwrites <Rs n="Entry"/> in the same <R n="subspace"/>. If you do not allow anyone else to write to your <R n="subspace"/>, it does not matter how devilishly clever they choose their timestamps — your data remains unaffected."),
            ...
            pinformative("User interfaces that rely on timestamps might still be coaxed into undesired behaviour, but that would be just as true if the timestamps came from an <R n="Entry"/>’s ", r("Payload"), " rather than its metadata. The issue of trusting timestamps in UI design (or using more secure alternatives) is removed from the underlying protocol."),

            pinformative("But what if you yourself create an <R n="Entry"/> with an outlandish ", r("entry_timestamp"), "? That’s where things become more problematic."),

            pinformative("If you accidentally backdate an <R n="Entry"/>, it will vanish once it comes into contact with your own non-buggy <Rs n="Entry"/> (whose ", r("entry_path"), " is a non-strict prefix). Frustrating, but no real way around it. Fortunately, the consequences are rather limited."),

            pinformative("If you accidentally assign a large ", r("entry_timestamp"), ", things are more problematic: it will overwrite all non-buggy <Rs n="Entry"/> (whose ", r("entry_path"), " it is a non-strict prefix), and the buggy <R n="Entry"/> will replicate through the network, deleting the non-buggy <Rs n="Entry"/> everywhere. To add insult to injury, you cannot create any new <Rs n="Entry"/> with accurate ", rs("entry_timestamp"), " (under the buggy ", r("entry_path"), ") until the buggily assigned time has passed. The only remedy is to get every device that stores the buggy <R n="Entry"/> (or <Rs n="Entry"/>) to locally delete them at the same time, before resuming data replication. This is quite an effort for small networks, and completely infeasible for large networks."),
        ...
        hsection("timestamps_meadowcap", "Timestamps in Meadowcap", [
            ...
            pinformative("If you give me a capability to write <Rs n="Entry"/> to your <R n="subspace"/> on the 18th of July 2024, I can still create <Rs n="Entry"/> in 2032 and add them to your <R n="subspace"/>, but only if I give them a ", r("entry_timestamp"), " for the 18th of July 2024. And you can make sure that no such <R n="Entry"/> will be propagated by creating your own <Rs n="Entry"/> whose ", r("entry_timestamp"), " is higher than that of the 18th of July 2024."),
            ...
            pinformative("The system of expiring capabilities does provide a good reason for future-dating entries: a moderator can irrevocably (for the original author) delete an <R n="Entry"/> by overwriting it with another <R n="Entry"/> whose ", r("entry_timestamp"), " is greater than the greatest time at which the original author can date their <Rs n="Entry"/>. Hence, peers should adjust the safeguard time difference they allow between new <Rs n="Entry"/> and their local clock to allow for moderation (for example by only rejecting <Rs n="Entry"/> whose time difference exceeds the longest issued capability lifetime by one extra minute)."),
        ]),
