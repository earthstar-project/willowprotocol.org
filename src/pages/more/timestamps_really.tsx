  ...
  <Dir name="timestamps_really">
    <File name="index.html">
      ...
      >
        ...
        {
          /*
        pinformative("Willow implements a key-value store (up to prefix-based overwrites), and a key-value store can map any one key to at most one value. Willow uses simple numeric <Rs n="Timestamp"/> to resolve conflicting mappings. This design decision might trigger some healthy scepticism, so this text walks you through our reasoning for building them into the protocol."),
        ...
        hsection("why_timestamps_anyways", "Why We Use Timestamps Anyways", [
            ...
            pinformative("Mapping a set of concurrently created <Rs n="Entry"/> to a single winner cannot be done in a fully satisfactory way, but we can at least try to improve the situation: by annotating every <R n="Entry"/> with its <R n="entry_timestamp"/>. ", em("If"), " the issuers are truthful and have sufficiently synchronised clocks, we get the expected outcome of the newest <R n="Entry"/> winning. That’s a lot better than taking a purist view and choosing, for example, the <R n="Entry"/> with the lowest <R n="entry_payload_digest"/>. In case of tied <Rs n="entry_timestamp"/>, we can safely fall back to the hash option (and distinct <Rs n="Payload"/> never hash to the same digest by assumption)."),

            pinformative("If an author is malicious, that author can deliberately choose a high timestamp to make its <R n="Entry"/> the winner. But that’s not a useful argument against timestamps, because ", em("any"), " deterministic solution (that gives all authors a fair treatment) can be gamed. If we decided by lowest hash, the malicious author could make semantically meaningless changes to their data until it hashed to a sufficiently low value."),
        ...
        hsection("rouge_timestamps", "Handling Rogue Timetamps", [
            ...
            pinformative("Willow automatically separates <Rs n="Entry"/> by ", r("entry_subspace_id"), ". An <R n="Entry"/> with a high <R n="entry_timestamp"/> only overwrites <Rs n="Entry"/> in the same <R n="subspace"/>. If you do not allow anyone else to write to your <R n="subspace"/>, it does not matter how devilishly clever they choose their timestamps — your data remains unaffected."),
            ...
            pinformative("User interfaces that rely on timestamps might still be coaxed into undesired behaviour, but that would be just as true if the timestamps came from an <R n="Entry"/>’s <R n="Payload"/> rather than its metadata. The issue of trusting timestamps in UI design (or using more secure alternatives) is removed from the underlying protocol."),

            pinformative("But what if you yourself create an <R n="Entry"/> with an outlandish <R n="entry_timestamp"/>? That’s where things become more problematic."),

            pinformative("If you accidentally backdate an <R n="Entry"/>, it will vanish once it comes into contact with your own non-buggy <Rs n="Entry"/> (whose <R n="entry_path"/> is a non-strict prefix). Frustrating, but no real way around it. Fortunately, the consequences are rather limited."),

            pinformative("If you accidentally assign a large <R n="entry_timestamp"/>, things are more problematic: it will overwrite all non-buggy <Rs n="Entry"/> (whose <R n="entry_path"/> it is a non-strict prefix), and the buggy <R n="Entry"/> will replicate through the network, deleting the non-buggy <Rs n="Entry"/> everywhere. To add insult to injury, you cannot create any new <Rs n="Entry"/> with accurate <Rs n="entry_timestamp"/> (under the buggy <R n="entry_path"/>) until the buggily assigned time has passed. The only remedy is to get every device that stores the buggy <R n="Entry"/> (or <Rs n="Entry"/>) to locally delete them at the same time, before resuming data replication. This is quite an effort for small networks, and completely infeasible for large networks."),

            pinformative("This is bad. It should only happen very rarely, but it still makes sense to implement additional countermeasures. First, any API to create <Rs n="Entry"/> of the current time can be hardcoded to raise an error if its time source reports a timestamp that unrealistically diverges from the implementation time. And second, peers can and should simply reject incoming <Rs n="Entry"/> that were created too far in the future compared to their local time. This way, buggy <Rs n="Entry"/> stay isolated on the offending device rather than polluting the network."),

            pinformative("An appropriate definition of ", quotes("too far in the future"), " depends on the expected clock-drift and can hence be fairly accurate. Rejecting <Rs n="Entry"/> ten minutes from the future should be more than enough to account for any clock drift that can be considered non-buggy. But there are good reasons for future-dating <Rs n="Entry"/> in the context of moderation, which brings us to the next topic: capabilities."),
        ]),
        ...
        hsection("timestamps_meadowcap", "Timestamps in Meadowcap", [
            ...
            pinformative("Willow allows write-access control via the <R n="is_authorised_write"/> function, which receives a full <R n="Entry"/> as input — including its <R n="entry_timestamp"/>. ", link_name("meadowcap", "Meadowcap"), " (the recommended access-control system for Willow) allows to incorporate <Rs n="entry_timestamp"/> into access control. In the following, we discuss issues of timestamps with respect to Meadowcap. We assume familiarity with (the introduction of) the ", link_name("meadowcap", "Meadowcap specification"), "."),

            pinformative("When a Meadowcap capability restricts <R n="Entry"/> creation based on timestamps, this has ", em("absolutely nothing"), " to do with the (physical) time at which <Rs n="Entry"/> are ", em("created"), ". It merely restricts which times can be ", em("claimed"), " as the <Rs n="entry_timestamp"/> of <Rs n="Entry"/>."),

            pinformative("If you give me a capability to write <Rs n="Entry"/> to your <R n="subspace"/> on the 18th of July 2024, I can still create <Rs n="Entry"/> in 2032 and add them to your <R n="subspace"/>, but only if I give them a <R n="entry_timestamp"/> for the 18th of July 2024. And you can make sure that no such <R n="Entry"/> will be propagated by creating your own <Rs n="Entry"/> whose <R n="entry_timestamp"/> is higher than that of the 18th of July 2024."),

            pinformative("Repeatedly handing out capabilities that grant access for a small time span imitates the concept of revoking rights in a more centralised environment: you simply do not renew someone’s capability, and they cannot create accurately timestamped <Rs n="Entry"/> once their last capability has expired. And by creating newer <Rs n="Entry"/>, you make sure that someone with an expired capability cannot make their writes propagate anymore."),
            ...
            pinformative("Write capabilities also happen to provide another layer of protection against accidentally future-dating your own <Rs n="Entry"/>. If my write capability reaches only one week into the future, I can start mitigating the damage done by an accidental future-dating after a single week. Furthermore, the buggy clock has to produce a time in the valid range, whereas other buggy timestamps will be filtered out immediately, leaving the <R n="subspace"/> unpolluted."),

            pinformative("The system of expiring capabilities does provide a good reason for future-dating entries: a moderator can irrevocably (for the original author) delete an <R n="Entry"/> by overwriting it with another <R n="Entry"/> whose <R n="entry_timestamp"/> is greater than the greatest time at which the original author can date their <Rs n="Entry"/>. Hence, peers should adjust the safeguard time difference they allow between new <Rs n="Entry"/> and their local clock to allow for moderation (for example by only rejecting <Rs n="Entry"/> whose time difference exceeds the longest issued capability lifetime by one extra minute)."),
        ]),
