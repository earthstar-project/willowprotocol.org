use willow_25::{Area, Entry, NamespaceId25, Path, PayloadDigest25, Range, Range3d, SubspaceId25};
use willow_data_model::{SubspaceId, grouping::AreaSubspace};
...
fn main() {
    ...
    println!(
        "any_late_blog_alfie includes alfie_late_blog_entry: ${:?}",
        any_late_blog_area.includes_entry(&alfie_late_blog_entry)
    );

    let music_path = Path::from_slices(&["music"]).unwrap();

    let alfie_music_area = Area::new(
    ...
    println!(
        "alfie_music_area includes alfie_early_blog_entry: ${:?}",
        alfie_music_area.includes_entry(&alfie_early_blog_entry)
    );

    println!(
        "alfie_music_area includes alfie_late_blog_entry: ${:?}",
        alfie_music_area.includes_entry(&alfie_late_blog_entry)
    );
}
